...
that results too young for being related to the client start point. This                        % GRAMMAR young? Isn't low or fresh better?
lower limit is more critical: a too low limit would guarantee false                             % GRAMMAR guarantee? Isn't "lend us to" better?
...
The analysis proceeds by recording a set of possible server candidates 
 per each client connection request. Each server candidate $s$ assumes a                        % GRAMMAR per each seems wrong to me, "for each" or "per"?
...
time-stamp of the server $s$ related to the entry time-stamp of the client connection request   % GRAMMAR related? Isn't "and" better? (between is a relation)
$creq$.
...
the server connection acceptance attempted far from 100ms.                                      % GRAMMAR attempted? Isn't "go far from the 100ms threshold" (or threshold min so we are relative) better?
...
than 10 minutes\cite{tormanual}.}.                                                              % GRAMMAR than? Isn't "maximum of 10 minutes" better?
Thus, as the $pmatch$ is defined as the $\Delta_t$ normalization, 
let us define the $gap$ average related to a                                                    % GRAMMAR Let us? Isn't "we defined" better?
...
where $N(c,s)$ is the number of $c$ connection requests that have been likely                   % GRAMMAR uppercase W
accepted from $s$.
...
their own score, they are sorted, in a descending order, by the score                           % GRAMMAR "they are sorted in descending order" without comma.
...
Let us notice that a candidate score highly depends by the number of connection                 % GRAMMAR Let us notice that? Isn't "As we can notice.."
...
information cannot be obtain by the client and server applications themselves                   % GRAMMAR obtain? Isn't obtained better?
but in a simulation environment, we can better figure out how a real                            % GRAMMAR better figure out? Isn't "figure out better" better? (yeah! Repetitions in grammar comments!)
...
how many traced clients found their right server.                                               % GRAMMAR "right server"? Is not the client that find the server...is the analysis that guess a server for a client, we may need to rewrite this phrase.
...
\begin{enumerate}
	\item The number of traced clients was fixed to almost 100\% of the                           % GRAMMAR almost looks like somewhat imprecise (in all this enumerate)
...
Each simulation run was executed on a dedicated machine which included                          % GRAMMAR included? Isn't "was composed of" better?
...
virtualized. One of the most essential point for future works would be to                       % GRAMMAR Isn't "...would be the replication of the experiments with..." better?
...
conducted from the clients side (it searches the right servers of the                           % GRAMMAR right server again, isn't relative server node better?
traced clients) and the matched portion is thus relative to the                                 % GRAMMAR of the traced clients, isn't for the traced clients (if we will decide to use "right server").
...
percentage of clients and servers actually logged by the analyzer plug-in on the                % GRAMMAR on? Isn't "over" better?
...
This behaviour is given simply because the logger entity is centralized                         % GRAMMAR given? Isn't "...behaviour is present because" better?
