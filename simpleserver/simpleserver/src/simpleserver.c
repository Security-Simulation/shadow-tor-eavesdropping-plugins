...
/* all state for simpleserver is stored here */
...
void prepareToTrace(int argc, char **argv, SimpleServer *h)
{
...
static int _simpleserver_startWriter(SimpleServer* h) {
...
static int _simpleserver_startReader(SimpleServer* h) {
...
static int _simpleserver_startAs(SimpleServer *h) {
	int ret;
	if ( (ret = _simpleserver_startWriter(h)) < 0 ) {
		/* Error case */
		return ret;
	}
	return _simpleserver_startReader(h);
}
...
SimpleServer* simpleserver_new(int argc, char* argv[], ShadowLogFunc slogf) {
	...
	SimpleServer* h = calloc(1, sizeof(SimpleServer));
	assert(h);
	...
	int isFail = 0;
	isFail = _simpleserver_startAs(h);
...
char *buildTracePackage(SimpleServer *h)
{
...
void closeConnections(SimpleServer *h, int sd, struct partner_s *partner)
{
	int lsd = sd;
 ...
 */
void handleNewConnection(SimpleServer *h, int sd)
{
	...
	/* XXX CHECK */
	if(h->isDone == 1) {
		if (_simpleserver_startWriter(h) < 0)
			exit(EXIT_FAILURE);
		h->isDone = 0;
	}
	/* END CHECK */
 ...
 */
int handleRead(SimpleServer *h, int sd, struct partner_s *partner)
{
 ...
 */
int handleWrite(SimpleServer *h, int sd, int outsd, struct partner_s *me)
{
	ssize_t sentbytes = -1;
...
static void _simpleserver_activateAs(SimpleServer* h, int sd, uint32_t events) {
...
void simpleserver_ready(SimpleServer* h) {
	...
	for(int i = 0; i < nfds; i++) {
		...
		_simpleserver_activateAs(h, d, e);
	}
}
